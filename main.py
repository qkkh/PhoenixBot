...
# نافذة التحكم بروم معين عن طريق الـ ID
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not target_channel:
                return await interaction.response.send_message("لم أتمكن من العثور على هذا الروم تأكد من الـ ID" ephemeral=True)
...
@bot.tree.command(name="dashboard" description="إرسال لوحة التحكم بالـ ID")
async def setup(interaction):
    if interaction.user.id == OWNER_ID:
        embed = discord.Embed(title="PHOENIX ID CONTROL" description="أدخل ID الروم للتحكم الكامل" color=0x00aaff)
