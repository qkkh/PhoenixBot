    ...
    async def on_member_join(self, member):
        ...
        if channel:
            ...
            try:
                ...
                print(f"Error drawing welcome image: {e}")
                await channel.send(welcome_text)

    # 2 تحديث إحصائيات السيرفر (كل 30 دقيقة)
...
# نظام التحميل المنظم (Embeds) مع دعم البيانات الخام
...
# 3 نظام النشر الاحترافي المعدل بالكامل
...
async def post(interaction: discord.Interaction, الافتار: str, البنر: str):
    if interaction.user.id == OWNER_ID or interaction.user.guild_permissions.manage_messages:
        ...
        try:
            ...
            # فتح الصور باستخدام PIL لتجنب خطأ Constructor parameter
...
# 4 الأوامر الإدارية (مسح قفل فتح حالة)
